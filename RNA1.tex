...
Funciones para entrenar mapas supervisadas y auto-organizados (SOM). También interrogatorio de los mapas y la predicción usando mapas entrenados son compatibles. El nombre del paquete que se refiere a Teuvo Kohonen, el inventor de la SOM.
\end{enumerate}
...
El perceptrón multicapa es una red neuronal artificial (RNA) formada por múltiples capas, esto le permite resolver problemas que no son linealmente separables, lo cual es la principal limitación del perceptrón simple. El perceptrón multicapa puede ser totalmente o localmente conectado. En el primer caso cada salida de una neurona de la capa "i" es entrada de todas las neuronas de la capa "i+1", mientras que en el segundo cada neurona de la capa "i" es entrada de una serie de neuronas (región) de la capa "i+1".
...
\item El proceso se debe repetir, volviendo a presentar todo el juego de patrones de aprendizaje E1, E2, .., un mínimo de 500 veces (t = 500).\\
\end{enumerate}
\end{enumerate}
...
\begin{itemize}
\item Un buen entendimiento de los conceptos acerca del tema de redes neuronales.
        
\item Hacer de los algoritmos de aprendizaje los más eficiente utilizando técnicas de programación.
        
\item Relacionar la biólogia computacional con la ciencias de la computación.
        
\item Conocer acerca del funcimiento del sistema nervioso mediante la simulación de las redes neuronales.
\end{itemize}
...
\bibitem{InteligenciaArtificial}
\newblock {R}ubén {C}ardenas {A}lmeida.
\newblock {I}nteligencia {A}rtificial, 2000 - 2001. 
\url{personales.upv.es/ccarrasc/doc/2000-2001/Redes Neuronales Excalibur - Nativ. Noverges/redes.htm#2}.
...
\bibitem{RedesNeuronalesBlog}
\newblock {M}arco {A}ntonio {C}arcini {M}ora.
\newblock {R}edes {N}euronales, 2012.
